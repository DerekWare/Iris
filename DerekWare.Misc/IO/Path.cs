...
namespace DerekWare.IO
{
    ...
    public partial class Path : IEquatable<Path>, IEquatable<object>, IComparable<Path>, IComparable<object>, IComparable
    {
            ...
            get
            {
                switch(GetPathFormat(this))
                {
                    ...
                    case PathFormat.Local:
                        if(!Url.Contains(InvalidPathChars) && !Url.Contains(new[] { "*", "?" }))
                        {
                            try
                            {
        ...
        {
            // d:\foo
            return (path?.Length > 2) && char.IsLetter(path[0]) && (':' == path[1]) && (('\\' == path[2]) || ('/' == path[2]));
        }
        ...
        public static IEnumerable<string> SplitPath(string path, StringSplitOptions options = default(StringSplitOptions))
        {
            return path.Split(PathDelimiters, options);
        }

        public static IEnumerable<string> SplitPath(IEnumerable<string> paths, StringSplitOptions options = default(StringSplitOptions))
        {
